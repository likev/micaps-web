...
describe("App Workflow & Preset Orchestration Integration (Phase 3 Gate)", () => {
  ...
  describe("Product Loading & Map Orchestration (TLOGP & Multi-Window)", () => {
    ...
    it("executes applyProductToWindow for TLOGP observation product with mock map and verifies loadTLogPLayer dispatch", async () => {
      const win = createDefaultWindow(0, 1);
      win.level = 500;
      ...
      const services = {
        clearAllWeatherLayersFromMap: (map, w) => {
          calls.cleared = true;
        },
        syncObservationTimeline: async (path, time, title, w) => {
          calls.syncedPath = path;
          return "20260921000000.000";
        },
        getOrCreateTimeline: (id) => {
          let tl = timelines.get(id);
          if (!tl) {
            tl = createTimelineState(id);
            timelines.set(id, tl);
          }
          return tl;
        },
        loadObservationProduct: async (map, model, elem, lvl, file, w, pth, ctg, srcId) => {
          calls.obsProduct = { model, elem, lvl, file, srcId };
        },
        loadTLogPLayer: async (map, layerDef, period, lvl, w) => {
          calls.tlogpLayer = layerDef;
        },
      };
      ...
      await import("../../src/lib/services/appWorkflow.js").then((m) =>
        m.applyProductToWindow(win, mockMap, product, services)
      );

      // Verify TLOGP exclusions & contract
      expect(calls.cleared).toBe(true);
      expect(calls.syncedPath).toBe("UPPER_AIR/TLOGP");
      expect(win.level).toBe(null); // Level must be stripped for full-sounding TLogP
      expect(win.isObservation).toBe(true);
      expect(win.title).toContain("TLOGP (UPPER_AIR)");
      expect(calls.obsProduct).toEqual({
        model: "UPPER_AIR",
        elem: "TLOGP",
        lvl: null,
        file: "20260921000000.000",
        srcId: "upperair-tlogp-stations",
      });
      expect(calls.tlogpLayer).toBeTruthy();
      expect(calls.tlogpLayer.id).toBe("upperair-tlogp-diagram");
      expect(calls.tlogpLayer.type).toBe("tlogp");
      expect(uiState.timelineVisible).toBe(true);
